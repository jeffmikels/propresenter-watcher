...
|
`);
server.listen(config.UI_SERVER_PORT, '0.0.0.0');
...
function loadLocalConfigFile() {
  ...
  try {
    ...
    console.log(
      `WARNING: Could not read local settings from ${CONF_FILE}, using defaults from repository`,
    );
  }
}
...
function registerAllConfigured() {
  ...
  for (let [name, mod] of Object.entries(modulesByName)) {
    if (mod.instances) {
      Log(`Clearing module instances for: ${name}`);
  ...
  for (let controllerName of Object.keys(config.controllers)) {
    if (!controllerName in modulesByName) continue;
    let controllerModule = modulesByName[controllerName];
    let coptions = config.controllers[controllerName];
    let cm;
    Log(`Reading configuration for controller: ${controllerName}`);
