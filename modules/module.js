...
class Module extends EventEmitter {
	...
	static create() { console.log( 'unimplemented' ) }

	// lets us read the static name from an instance
	get moduleName() { return this.constructor.name; }
	get niceName() { return this.constructor.niceName; }
	get supportsMultiple() { return this.constructor.supportsMultiple; }
	get instanceName() { return this._instanceName ?? this.id ?? this.uuid; }
		...
		if ( config.triggers ) {
			for ( let t of config.triggers ) {
				this.registerTrigger( new ModuleTrigger(
					t.tagname,
					t.description,
					t.args.map( e => new ModuleTriggerArg(
						e.name,
						e.type,
						e.description,
						e.optional
...
class ModuleTrigger {
	...
	fireIfEnabled( incomingArgs, ppInstance ) {
		...
		if ( this.parent && this.parent.supportsMultiple && this.parent.instanceName != parsed[ 0 ] ) return false;
	...
	constructor ( name = '', type = 'number', description = '', optional = false ) {
		...
		switch ( this.type ) {
			...
			case 'bool':
				...
				break;
			case 'string':
				this.help = 'Strings with commas must be surrounded by quotation marks. You may use single quotes (\'), double quotes ("), or backticks (`).'
