...
class MidiController extends Module {
	...
	static create( config ) { return new MidiController( config ); }
	constructor ( config ) {
		super( config );
		...
		this.registerTrigger( new ModuleTrigger(
			'note',
			'play a midi note to the connected port',
			[
				new ModuleTriggerArg( 'note', 'number', 'number from 0-127', false ),
				new ModuleTriggerArg( 'velocity', 'number', 'velocity from 0-127 defaults to 127', true ),
				new ModuleTriggerArg( 'channel', 'number', 'channel defaults to 0', true ),
			],
			( _, note, velocity = 127, channel = 0 ) => this.hit( note, velocity, channel ),
		) );
		...
		this.registerTrigger( new ModuleTrigger(
			...
			[
				new ModuleTriggerArg( 'controller', 'number', 'number from 0-127 (silence all by setting cc 120 and cc 123 to 0)', false ),
				new ModuleTriggerArg( 'value', 'number', 'value from 0-127 defaults to 127', true ),
				new ModuleTriggerArg( 'channel', 'number', 'channel defaults to 0', true ),
			],
			( _, controller, value = 127, channel = 0 ) => this.control( controller, value, channel ),
		) );
		...
		this.registerTrigger( new ModuleTrigger(
			...
			[
				new ModuleTriggerArg( 'initial', 'string', 'HH:MM:SS:FF where FF is frame number', true ),
				new ModuleTriggerArg( 'fps', 'number', 'defaults to 24', true ),
			],
			( _, initial, fps = 24 ) => initial == null ? this.mtcStop() : this.mtcStart( initial, fps ),
		) );
	}
