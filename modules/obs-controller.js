...
class OBSController extends Module {
	...
	static create( config ) { return new OBSController( config ); }
	...
	async connect() {
		...
		this.obs.connect( { address, password } ).then( () => {
			this.connected = true;
	...
	async setStudioMode( onoff = true ) {
		return onoff ? await this.obs.send( 'EnableStudioMode' ) : await this.obs.send( 'DisableStudioMode' );
	}
	...
	async transitionToScene( transition = null, scene = null, duration = null ) {
		...
		if ( duration != null )
			cmd.SetTransitionDuration = { duration };
