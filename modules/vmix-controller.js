...
class VmixController extends Module {
	...
	static create( config ) { return new VmixController( config ); }
	...
	constructor ( config ) {
		...
		this.registerTrigger(
			new ModuleTrigger(
				...
				( _, trans, input, duration ) => this.transitionToInput( trans, input, duration ),
			),
		);
		...
		this.registerTrigger(
			new ModuleTrigger(
				...
				( pro, input, selection = 0, override = null ) => this.setInputText( input, override ?? pro.slides.current.text, selection ),
			),
		);
		...
		this.registerTrigger(
			new ModuleTrigger(
				...
				( _, overlaynum = 1, toggletype = 0, input = null, seconds = null ) => this.setOverlay( overlaynum, toggletype, input, seconds ),
			),
		);
		...
		this.registerTrigger(
			new ModuleTrigger(
				...
				( _, start = true, stream_number = 1 ) => this.triggerStream( start, stream_number ),
			),
		);
	...
	setOverlay( overlay = 1, type = '', input = null, seconds = null ) {
		...
		if ( seconds != null && ( type == 'In' || type == 'On' ) ) {
			...
			this.previousTimer = setTimeout( () => this.setOverlay( overlay, flipped, input ), seconds * 1000 );
		}
	}
